...
# The time at which the program starts (real time)
start_real_time = time.time()

# The time at which the program starts (in the program's "compressed" time)
start_prog_time = datetime.now()


def get_current_time():
    # How much real time has passed since the program started?
    real_time_elapsed = time.time() - start_real_time

    # In the program's "compressed" time, 24 hours pass in 60 seconds.
    # So, if X seconds of real time have passed, then X * (24 hours / 60 seconds)
    # of compressed time have passed.
    prog_time_elapsed = real_time_elapsed * (24 * 60 * 60) / 60

    # The current time in the program is the start time plus the elapsed time
    current_prog_time = start_prog_time + timedelta(seconds=prog_time_elapsed)
    return current_prog_time
...
class Sunset(SampleBase):
    ...
    def plot_data(self, data, color=(255, 255, 255), lower_limit=10, upper_limit=0):
        ...
        values = [None if value == "nodata" or value ==
                  "null" else float(value) for value in values]
        ...
        normalized_values = [lower_limit - (value - min_value) / (max_value - min_value) * (
            lower_limit - upper_limit) if value is not None else None for value in values]

        # Step 3: Iterate over data and plot
        last_valid_y = None
        for i in range(1, len(dates)):
            x1 = (dates[i - 1] - dates[0]).days / \
                (dates[-1] - dates[0]).days * (int(panel_width) - 1)
            x2 = (dates[i] - dates[0]).days / (dates[-1] -
                                               dates[0]).days * (int(panel_width) - 1)
    ...
    def get_sun_position(self, current_time):
        ...
        screen_position = abs(
            panel_height-int((panel_height / 2) * (1-sun_elevation)))  # abs to invert
    ...
    def update_sky_color(self, offset):
        ...
        # extra offset to fix bug
        future_time = future_time - timedelta(hours=6)

        # Normalize the time to a value between 0 (midnight) and 1 (next midnight)
        t = ((future_time.hour * 60 + future_time.minute) * 60 +
             future_time.second) / 86400  # 86400 seconds in a day
        ...
        color = [c1 * (1 - t) + c2 * t for c1, c2 in zip(colors[i %
                                                                len(colors)], colors[(i + 1) % len(colors)])]
